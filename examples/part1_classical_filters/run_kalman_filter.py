...
def run_experiment():
    ...
    # 2. Create model and filter
    model = LinearGaussianSSM(
        F=tf.constant(F), H=tf.constant(H),
        Q=tf.constant(Q), R=tf.constant(R),
    )
    kf = KalmanFilter(model)

    # 3. Generate Synthetic Data (Ground Truth & Observations)
    steps = 100
    true_states = []
    measurements = []

    current_x = x_init.copy().reshape(-1, 1)
    np.random.seed(42)
    ...
    # 4. Run Kalman Filter
    obs_tensor = tf.constant(
        np.array([m.flatten() for m in measurements]), dtype=tf.float32
    )
    estimates, covariances = kf.run(obs_tensor, tf.constant(x_init), tf.constant(P_init))

    print(f"Processed {steps} time steps.")

    # 5. Visualization
    true_pos = [s[0, 0] for s in true_states]
    est_pos = [estimates[t, 0, 0].numpy() for t in range(steps)]
    meas_pos = [m[0, 0] for m in measurements]

    std_devs = [np.sqrt(covariances[t, 0, 0].numpy()) for t in range(steps)]
    ...
    plt.fill_between(range(steps), lower_bound, upper_bound,
                     color='blue', alpha=0.2, label='95% Confidence')

    plt.title('Part 1.1.a: Multidimensional Linear-Gaussian SSM (TensorFlow)\n'
